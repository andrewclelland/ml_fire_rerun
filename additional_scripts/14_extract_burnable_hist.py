...
IN_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/new_training")

# UPDATED: Point to the new prediction-only shapefiles
PRED_SHP_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/new_training/stage_1_model_new_fwi/predictions_only_shapefiles_annual_new_fwi")

# UPDATED: Output directory for the new dataset (renamed to reflect it's masked by predictions)
OUT_DATASET_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/new_training/parquet_historical")
    ...
    for year, month, tif in tqdm(todo, desc="Building partitioned Parquet dataset (Stage 1 Pred mask)"):
        ...
        with rio.open(tif) as ds:
           ...
           # Subset pixels
            ...
            # Dynamically find the precipitation column and multiply by 1,000,000
            #precip_col = next((c for c in df.columns if "precipitation" in c.lower()), None)
            #if precip_col:
            #    df[precip_col] = df[precip_col] * 1_000_000.0
