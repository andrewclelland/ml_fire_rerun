...
OUT_DIR = "/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/MRI-ESM2-0/ssp370"
...
monthly_tifs = sorted(Path(OUT_DIR).glob("MRI-ESM2-0_ssp370_*.tif"))
...
for year in sorted(year_to_paths.keys()):
    ...
    expected_parquet = PARQUET_DIR / f"MRI-ESM2-0_ssp370_{year}_annual_grid1deg.parquet"
    ...
    with rio.open(month_paths[0][1]) as ds_template:
        ...
        aggregate_to_coarse_grids_annual(
            ...
            GRID_SIZES_DEG, BURNED_THRESHOLD, PARQUET_DIR, COARSE_TIF_DIR, COARSE_SHP_DIR, f"MRI-ESM2-0_ssp370_{year}_annual"
        )
