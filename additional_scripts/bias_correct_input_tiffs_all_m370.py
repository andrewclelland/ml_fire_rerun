...
for scenario in ['ssp370']:
    input_dir = f"/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_raw/MRI-ESM2-0/{scenario}"
    bias_dir = "/gws/ssde/j25b/bas_climate/users/clelland/model/bias_correction_factors/"
    output_dir = f"/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/MRI-ESM2-0/{scenario}"
    ...
    def find_bias_file(month):
        pattern = os.path.join(bias_dir, f"bias_correction_factors_MRI-ESM2-0_{scenario}_{month:02d}.tif")
    ...
    # MAIN LOOP
    # -----------------------------
    input_files = glob.glob(os.path.join(input_dir, "*209*.tif"))
