...
IN_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/ACCESS-CM2/ssp370")

# UPDATED: Point to the new prediction-only shapefiles
PRED_SHP_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/ACCESS-CM2/ssp370/predictions_only_shapefiles_annual")

# UPDATED: Output directory for the new dataset (renamed to reflect it's masked by predictions)
OUT_DATASET_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/parquets_indiv/parquet_ACCESS-CM2_ssp370")
...
name_re = re.compile(r"ACCESS-CM2_ssp370_(\d{4})_(\d{1,2})\.tif$", re.IGNORECASE)
# UPDATED: Shapefile regex to match the new prediction-only naming convention
shp_re = re.compile(r"ACCESS-CM2_ssp370_(\d{4})_annual_grid1deg_predictions_only\.shp$", re.IGNORECASE)
...
# ================== MAIN ==================
def main():
    tifs = sorted(IN_DIR.glob("ACCESS-CM2_ssp370_*.tif"))
    if not tifs:
    ...
    for year, month, tif in tqdm(todo, desc="Building partitioned Parquet dataset (Stage 1 Pred mask)"):
        ...
        shp_name = f"ACCESS-CM2_ssp370_{year}_annual_grid1deg_predictions_only.shp"
