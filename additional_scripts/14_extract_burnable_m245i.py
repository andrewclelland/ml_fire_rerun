...
IN_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/MRI-ESM2-0/ssp245")

# UPDATED: Point to the new prediction-only shapefiles
PRED_SHP_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/testing_inputs_final_indiv/MRI-ESM2-0/ssp245/predictions_only_shapefiles_annual")

# UPDATED: Output directory for the new dataset (renamed to reflect it's masked by predictions)
OUT_DATASET_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/parquets_indiv/parquet_MRI-ESM2-0_ssp245")
...
name_re = re.compile(r"MRI-ESM2-0_ssp245_(\d{4})_(\d{1,2})\.tif$", re.IGNORECASE)
# UPDATED: Shapefile regex to match the new prediction-only naming convention
shp_re = re.compile(r"MRI-ESM2-0_ssp245_(\d{4})_annual_grid1deg_predictions_only\.shp$", re.IGNORECASE)
...
# ================== MAIN ==================
def main():
    tifs = sorted(IN_DIR.glob("MRI-ESM2-0_ssp245_*.tif"))
    if not tifs:
    ...
    for year, month, tif in tqdm(todo, desc="Building partitioned Parquet dataset (Stage 1 Pred mask)"):
        ...
        shp_name = f"MRI-ESM2-0_ssp245_{year}_annual_grid1deg_predictions_only.shp"
