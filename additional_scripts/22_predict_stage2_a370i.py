...
OUT_DIR = Path("/gws/ssde/j25b/bas_climate/users/clelland/model/inference_outputs_indiv/monthly_probs_raw")
MONTHLY_PROB_DIR = OUT_DIR / "ACCESS-CM2_ssp370"
...
# INFERENCE LOOP
# ----------------------------------------------------------------------
all_tifs = sorted(IN_DIR.glob("ACCESS-CM2_ssp370_*.tif"))
model_cache = {}
...
for i, tif_path in enumerate(all_tifs, 1):
    match = re.search(r"ssp370_(\d{4})_(\d+)", tif_path.name)
    if not match:
        continue
